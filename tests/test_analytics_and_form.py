...
def test_analytics_endpoints_return_expected_shapes():
    ...
    summary_response = client.get(f"/v1/analytics/summary/{user_id}")
    ...
    progress_response = client.get(f"/v1/analytics/progress/{user_id}")
    ...
    score_response = client.get(f"/v1/analytics/progress-score/{user_id}")
...
def test_form_analyze_returns_feedback_and_scores():
    ...
    response = client.post("/v1/form/analyze", json=payload)
...
def test_workout_history_timeline_includes_sets_and_rest_seconds():
    ...
    response = client.get(f"/v1/workouts/history/{user_id}")
    assert response.status_code == 200
    body = response.json()
